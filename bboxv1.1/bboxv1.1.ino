...
void loop() {
  ...
  if (mode != modeState) {
    modeTState = toggler(mode,modeTState);
    modeState  = mode;
    ui_drawMode();
  }
  ...
  if (sum != checksum) {
    ...
    //_____State vars______________
    for (int i = 0; i <= 24; i++) {
      btnState[i] = btn[i];
    }
    //_____Checksum_______________________
    checksum = 0;
    for (int i = 0; i <= 24; i++) {
...
void toggleRelay(int btn, int btnState, int relayID){
  switch (relayID) {
    case 1: relayState[0] = toggler(btn,relayState[0]);
            switch (relayState[0]) {
            ...
            delay(250);
            break;
    case 2: relayState[1] = toggler(btn,relayState[1]);
            switch (relayState[1]) {
